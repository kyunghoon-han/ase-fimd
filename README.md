...
## Use as an ASE-native integrator

`FIMDynamics` subclasses ASE's `MolecularDynamics`, so it plugs into existing ASE
workflows exactly like `ase.md.Langevin` or `VelocityVerlet` — the same
`run` / `attach` / `logfile` / `trajectory` interface. The only extra argument is
the modal `basis`, which is intrinsic to the method (FIMD propagates a chosen
...
# Build the modal basis once (from a Hessian or a reference trajectory):
basis = FIMDBasis.from_trajectory(reference_frames, reference, calculator,
                                  band=(0.0, 200.0))

# Then drive it like any ASE integrator:
md = FIMDynamics(
    atoms,
    basis,                  # the one FIMD-specific argument
    2 * units.fs,           # timestep, ASE-style
    temperature=300,        # omit (or None) for NVE; set a value (K) for an NVT band thermostat
    friction=0.01,
    logfile="run.log",
)
...
A complete, self-checking example lives in
[`examples/ase_native_interface.py`](examples/ase_native_interface.py).
